require_relative 'spec_helper'

describe RubySyntaxUpgrader::UpgradeHashRocketSyntax do

  let!(:old_controller_example) do
    """
      get :index
      get :index, :store => store.id
      post :index, :store => store.id
      patch :index, :store => store.id
      put :index, :store => store.id
      delete :index, :store => store.id
      get :index, :store => store.id, :box_type => reservation.box_type.id
      get :index, store: store.id, box_type: reservation.box_type.id
      get :index, :store => store.id, :sort => 'box_type', :direction => 'desc'
      put :update, id: 9999
      put :update, params: { id: 9999 }
      put :update, :id => reservation.id, :reservation => 
        { :box_type_id => store_2.box_types.last.id }

    """
  end



  let!(:new_controller_example) do
    """
      get :index
      get :index, params: { :store => store.id }
      post :index, params: { :store => store.id }
      patch :index, params: { :store => store.id }
      put :index, params: { :store => store.id }
      delete :index, params: { :store => store.id }
      get :index, params: { :store => store.id, :box_type => reservation.box_type.id }
      get :index, params: { store: store.id, box_type: reservation.box_type.id }
      get :index, params: { :store => store.id, :sort => 'box_type', :direction => 'desc' }
      put :update, params: { id: 9999 }
      put :update, params: { id: 9999 }
      put :update, params: { :id => reservation.id, :reservation => 
        { :box_type_id => store_2.box_types.last.id } }
    """
  end

  let(:hash_rocket_example_path) { './spec/temp' }
  let(:hash_rocket_example_file) { "#{hash_rocket_example_path}/example_file.rb" }

  before do
    @source = File.open(hash_rocket_example_file, 'w') do |f|
                f.write(hash_rocket_example)
              end
  end

  after do
    File.open(hash_rocket_example_file, 'w') do |f|
      f.write('')
    end
  end